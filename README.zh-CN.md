...
- **macOS**：下载 `CodexScope-mac.zip`，解压后双击根目录的 `open-dashboard.command`
- **Windows**：下载 `CodexScope-windows.zip`，解压后双击根目录的 `open-dashboard.cmd`
...
- `dist/CodexScope-mac.zip`：包含根目录 `open-dashboard.command` 和预编译 `codexscope-darwin-arm64`
- `dist/CodexScope-windows.zip`：包含根目录 `open-dashboard.cmd` 和预编译 `codexscope-windows-amd64.exe`
